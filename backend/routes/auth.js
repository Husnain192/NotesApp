...
const JWT_SECRET = 'Husnainisagoodboy';

// ROUTE 1: Create a user using: POST "/api/auth", Does not require Auth. No login required
router.post('/createuser', [
    ...
    let success = false;
    // If there are errors, return abd request and the error
...
router.post('/login', [
    body('email', 'Enter a valid name').isEmail(),
    body('password', 'Password must be atleast 1 character').exists()
] , async (req, res)=>{
    let success = false;
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
